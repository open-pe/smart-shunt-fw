...
bool PowerSampler_TMP117::init() {
    ...
    if (address < ADDR_MIN || address > ADDR_MAX) {
        ESP_LOGE(TAG, "bus %hhu address 0x%02hhX outside TMP117 range 0x%02X..0x%02X",
                 i2cPort, address, ADDR_MIN, ADDR_MAX);
        return false;
    }
    ...
    if (id != DEVICE_ID) {
        ESP_LOGE(TAG, "bus %hhu 0x%02hhX: DEVICE_ID 0x%03X, expected 0x%03X",
                 i2cPort, address, id, DEVICE_ID);
        return false;
    }
    ...
    if (i2c_write_short(i2cPort, address, REG_CONFIGURATION, CONFIG_SOFT_RESET) != ESP_OK) {
        ESP_LOGE(TAG, "bus %hhu 0x%02hhX: soft reset failed", i2cPort, address);
        return false;
    }
    delay(5);

    // EEPROM stays locked, so this write is volatile and costs no EEPROM cycles.
    if (i2c_write_short(i2cPort, address, REG_CONFIGURATION, CONFIG) != ESP_OK) {
        ESP_LOGE(TAG, "bus %hhu 0x%02hhX: config write failed", i2cPort, address);
        return false;
    }

    if (i2c_read_buf(i2cPort, address, REG_CONFIGURATION, buf, 2) != ESP_OK) {
        ESP_LOGE(TAG, "bus %hhu 0x%02hhX: config readback failed", i2cPort, address);
        return false;
    }
    const uint16_t cfg = ((uint16_t) buf[0] << 8 | buf[1]) & CONFIG_RW_MASK;
    if (cfg != CONFIG) {
        ESP_LOGE(TAG, "bus %hhu 0x%02hhX: config is 0x%04X, wrote 0x%04X",
                 i2cPort, address, cfg, CONFIG);
        return false;
    }

    ESP_LOGI(TAG, "bus %hhu 0x%02hhX: init ok (id=0x%03X, config=0x%04X)",
             i2cPort, address, id, cfg);
    return true;
}

float PowerSampler_TMP117::readTemperature() {
    uint8_t buf[2];
    if (i2c_read_buf(i2cPort, address, REG_TEMP_RESULT, buf, 2) != ESP_OK) {
        ++readErrors;
        if (shouldLog(readErrors)) ESP_LOGE(TAG, "bus %hhu 0x%02hhX: temp read failed (%lu)",
                                            i2cPort, address, readErrors);
        return NAN;
    }
    ...
    if (!(t >= TEMP_MIN && t <= TEMP_MAX)) {
        ++readErrors;
        if (shouldLog(readErrors)) ESP_LOGE(TAG, "bus %hhu 0x%02hhX: temp %.4f out of range (%lu)",
                                            i2cPort, address, t, readErrors);
        return NAN;
    }
...
bool PowerSampler_TMP117::hasData() {
    ...
    if (i2c_read_buf(i2cPort, address, REG_CONFIGURATION, buf, 2) != ESP_OK) {
        ++readErrors;
        if (shouldLog(readErrors)) ESP_LOGE(TAG, "bus %hhu 0x%02hhX: config poll failed (%lu)",
                                            i2cPort, address, readErrors);
